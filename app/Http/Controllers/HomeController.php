...
class HomeController extends Controller
{
    ...
    public function profile(Request $request){
       ...
       $selectedService = Auth::user()->services;
       $selectedService = array_column($selectedService->toarray(),'service_id');
       return view('profile',compact('services','selectedService'));
    }
   ...
   public function addService(Request $request){
       ...
       DB::beginTransaction();
       try{
         DB::table('vendor_services')->where('vendor_id',auth::id())->delete();
         DB::table('vendor_services')->insert($storeData);
         DB::commit();
         return back()->with('status',true)->with('message','Successfully updated service');
      }catch(\Exception $e){
        DB::rollback();
         return back()->with('status',true)->with('message','Failed to update service');
       }
   }
    ...
    public function deleteProduct(Request $request){
      ...
      $product->deleted_at     = date('Y-m-d H:i:s');
       if($product->update()){
          ...
          return redirect()->route('products')->with('status',true)->with('message',__('Successfully deleted product'));
       }
       return redirect()->route('products')->with('status',false)->with('message',__('Failed to delete product'));
    }
    ...
    public function buyPlan(Request $request){
      ...
      if($plan){
          ...
          if($insertId){
            ...
            return redirect()->route('home')->with('status',true)->with('message',__('Your plan actived'));
          }else{
