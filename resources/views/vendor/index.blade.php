...
@section('title') {{__('Vendor List')}} @endsection

@section('content')
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">{{__('Vendors')}} ({{$data['vendors']->total()}})</h3>
    </div>
</div>
...
<div class="row">
    <div class="col-md-10">
         <form class="form-inline">
              ...
              <a href="{{route('index.vendor')}}" class="btn btn-default">{{__('Clear')}}</a>
              <input type="submit" class="btn btn-primary" value="Submit"/>
              <a href="{{route('export.vendor',['search'=>Request::get('search'),'status'=>Request::get('status')])}}" class="btn btn-primary">{{__('Export')}}</a>
         </form>
    </div>
    <div class="col-md-2">
       <a href="{{route('create.vendor')}}" class="btn btn-primary btn-sm">{{__('Add Vendor')}}</a>
    </div>
</div>
<br/>
<div class="row">
                ...
                <tbody>
                    ...
                    @foreach ($data['vendors'] as $key => $value)
                        <tr class="odd gradeX">
                            ...
                            <td>{{ $value->totalBookings($value->id) }}</td>
                            <td class="center">{{$value->is_active == '1' ? __('Active') : __('Deactive') }}</td>
                            <td>
                                <a href="{{route('show.vendor',$value->id)}}" class="btn btn-sm btn-primary">{{__('Info')}}</a>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
                ...
                {{ method_field('DELETE') }}
                 ...
                 <label><input type="checkbox" checked name="is_notify" value="1"/>&nbsp;&nbsp;&nbsp;{{__('Notify to vendor')}}</label>
        </div>
